...
def counting_sort(nums): #O(n) time complexity and O(n) space complexity
    # get the min and max from the entire input and create a array of that range initialized to 0
    low = nums.get_min()
    high = nums.get_max()
    count_array = [0 for i in range(low, high+1)]
    # hash each of the input numbers into the corresponding index in the helper array
    for i in range(nums.get_len()):
        count_array[i-low] += 1
    i = 0
    # Traverse the helping array and reassign the input based on the values corresponding to each index in helping array
